...
public class Main extends TelegramLongPollingBot {
    ...
    public String getBotUsername() {
        return "UkraineBanderogusGoitBot";
    }

    @Override
    public String getBotToken() {
        return "6166486953:AAHUK_dzuGov7HgCpKhqbt_W1IfbSypx28E";
    }
    ...
    public void onUpdateReceived(Update update) {
       ...
       if(update.hasMessage()&&update.getMessage().getText().equals("/start")){
           ...
           buttons=getRandom(buttons);

           attachButtons(message,Map.of(buttons.get(0),"l1",
                   buttons.get(1),"l1",
                   buttons.get(2),"l1" ));
        ...
        if (update.getCallbackQuery().getData().equals("l1")&&getLevel(chatId)==1) {
            setLevel(chatId,2);
            ...
            buttons=getRandom(buttons);
            attachButtons(message,Map.of(buttons.get(0),"l2",
                    buttons.get(1),"l2",
                    buttons.get(2),"l2" ));
            sendApiMethodAsync(message);
        }

            if (update.getCallbackQuery().getData().equals("l2")&&getLevel(chatId)==2) {
                setLevel(chatId,3);
                ...
                buttons=getRandom(buttons);
                attachButtons(message,Map.of(buttons.get(0),"l3",
                        buttons.get(1),"l3",
                        buttons.get(2),"l3" ));
                sendApiMethodAsync(message);
            }
        if (update.getCallbackQuery().getData().equals("l2")&&getLevel(chatId)==3) {
            setLevel(chatId,4);
            ...
            message.setChatId(chatId);
            attachButtons(message,Map.of("Купити Джавелін (50 монет)","l4"));
            sendApiMethodAsync(message);
        }
        if (update.getCallbackQuery().getData().equals("l2")&&getLevel(chatId)==4) {
            setLevel(chatId,5);
